...
router.get('/opening-times-start', (_, res) => {
  res.render('profile-manager/dentists/editor/opening-times/opening-times-start', { recentOpeningTimesChange});
});

router.get('/days', (_, res) => {
  res.render('profile-manager/dentists/editor/opening-times/days', {
...
router.get('/days/:day', (req, res) => {
  ...
  res.render('profile-manager/dentists/editor/opening-times/day', { dayObj });
});
...
router.post('/days/:day/set', (req, res) => {
  ...
  if (req.body.open === 'yes') {
    ...
    delete times['open']
    res.render('profile-manager/dentists/editor/opening-times/copy', {
      dayObj,
      daysToDisplay,
      times,
    });
    ...
    res.redirect('/profile-manager/dentists/editor/opening-times/days');
  }
});
...
router.post('/days', (req, res) => {
  ...
  res.redirect('/profile-manager/dentists/editor/opening-times/confirm-opening-times');
});

router.get('/confirm-opening-times', (_, res) => {
  res.render('profile-manager/dentists/editor/opening-times/confirm-opening-times', {
    openingTimes: JSON.parse(localStorage.getItem('localOpeningTimes')),
  });
});

router.post('/confirm-opening-times', (_, res) => {
  res.redirect('/profile-manager/dentists/editor/opening-times/opening-times-start');
});
...
router.post('/bank-holiday/day', (req, res) => {
  ...
  res.render('profile-manager/dentists/editor/opening-times/bank-holiday/confirm', {
    firstTime,
    secondTime,
    thirdTime,
  });
...
router.get(
  ...
  (req, res) => {
    ...
    if (tempChanges.length === 0) {
      res.redirect(
        '/profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes-date'
      );
    } else {
      res.render('profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes', {
        tempChanges,
      });
    }
  }
...
router.get('/profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes-date',
  (req, res) => {
    recentOpeningTimesChange = false;
...
router.post('/temporary-changes/temporary-changes-date',
  (req, res) => {
    res.redirect('/profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes-open');
  }
);

router.post('/temporary-changes/temporary-change-open', (req, res) => {
  if (req.body.open === 'yes') {
    res.redirect(`/profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes-time`);
  } else {
    res.redirect('/profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes-range-question')
  }
});

router.post('/temporary-changes/temporary-changes-range-question', (req, res) => {
  if (req.body.tempChangeMultiDay === 'yes') {
    res.redirect('/profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes-done-multiple-days');
  } else {
    res.redirect('/profile-manager/dentists/editor/opening-times/temporary-changes/temporary-changes-done-one-day');
  }
});
